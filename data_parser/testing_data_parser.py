...
def data_parser():
    ...
    with open("testing_data.csv", "r", encoding="utf8")as rfh, open("machine_testing_data.csv", "w+",
        ...
        for cols in reader:
            ...
            writer.writerow([
                url, image_url, title, ','.join(map(str, location_country)),
                ','.join(map(str, location_type)), sub_title, video_url,
                ','.join(map(str, category)), ','.join(map(str, category_slug)),
                ','.join(map(str, currency)), funding_goal, duration_of_campaign,
                num_updates, num_comments, length_story, story_sentiment.subjectivity,
